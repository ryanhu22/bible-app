...
function Passage({ passageHTML, searchFunc, book, chapter }) {
  ...
  // Refs
  const observer = useRef(null); // Used for deleting comments
  ...
  // Utility for deleting comments
  useEffect(() => {
    observer.current = new MutationObserver(() => {
      // Do something when the document body changes
      const lastElement = document.body.lastElementChild;
      const elementType = lastElement.tagName;
      const elementText = lastElement.textContent;

      // If elementType is H5, then delete the highlight
      if (elementType === "H5") {
        // Loop through clearHighlightArray
        clearHighlightArray.map(([id_val, clearFunction]) => {
          if (id_val === elementText) {
            clearFunction();
            // Remove the comment popup too
            const body = document.querySelector("body");
            const lastDiv = body.lastChild;
  ...
  const saveHighlightsToDB = async (user_email, book, chapter, curr_html) => {
  ...
  const getNextPage = async (e) => {
    ...
    console.log(propRef.current.outerHTML);
    const curr_html = propRef.current.outerHTML;
    if (session?.user) {
      await saveHighlightsToDB(session.user.email, book, chapter, curr_html);
    }
  ...
  const inputPositionStyle = {
    top: inputPopupY - 30,
    left: inputPopupX >= 400 ? (1 / 2) * inputPopupX : inputPopupX,
  };
  ...
  //   setClearHighlightArray((prevArray) => {
  //     prevArray.map(([id_val, clearFunction]) => {
  //       if (id_val === "tempHighlight") {
  //         console.log("clearing");
  //         clearFunction();
  //       }
  //     });
  //     return prevArray.filter(
  //       ([id_val, clearFunction]) => id_val !== "tempHighlight"
  //     );
  //   });
  // };

  const submitComment = (e, commentInput) => {
    // Need to call the addComment with the range, but most times the selection is disappeared. Have a state variable to keep track onChange (useEffect)
    e.preventDefault();
    commentInput = commentInput.trim();
    const uniqueId =
      Date.now().toString(36) + Math.random().toString(36).substring(2, 5);

    // Find invisible commnent placeholder and update it
    // const all_spans = document.querySelectorAll("span");
    // const lastH4 =
    //   document.body.querySelectorAll("h4")[
    //     document.body.querySelectorAll("h4").length - 1
    //   ];
    // for (let i = 0; i < all_spans.length; i++) {
    //   let currentSpan = all_spans[i];
    //   if (currentSpan.id === lastH4?.textContent) {
    //     console.log(currentSpan);
    //     // currentSpan.style.backgroundColor = "blue";
    //     currentSpan.onClick = `this.commentClick("${uniqueId}")`;
    //     // currentSpan.addEventListener("click", `commentClick("${uniqueId}")`);
    //     currentSpan.onMouseEnter = `showCommentScriptFunc("${commentInput}", ${inputPopupX}, ${inputPopupY})`;
    //     currentSpan.onMouseLeave = `hideCommentScriptFunc(${inputPopupX}, ${inputPopupY})`;
    //   }
    // }
        ...
        <div>
          <div>
            ...
            <div className="flex" onMouseUp={handleSelection}>
              ...
              <div className="px-3" ref={propRef}>
                {ReactHtmlParser(passageHTML)}
              </div>
              ...
              <Helmet>
                ...
                <script>
                  {`function commentClick(commentID) {
                      const invisibleToDeleteElement = document.createElement('h5');
                      invisibleToDeleteElement.style.display = 'none';
                      invisibleToDeleteElement.textContent = commentID;
                      document.body.appendChild(invisibleToDeleteElement);
                  }`}
                </script>
              </Helmet>
              ...
              <Helmet>
              {/* Adding not to document body, but to HTMLparser */}
                <script>
